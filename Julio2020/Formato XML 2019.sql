...
DECLARE
	...
	v_MessageRefId					VARCHAR2(24 BYTE) := 'PE20192011106501300001';
	v_MessageTypeIndic				VARCHAR2(6 BYTE) := 'CRS701';
	v_ReportingPeriod				DATE := TO_DATE('2019-12-31', 'YYYY-MM-DD');
...
BEGIN
	...
	If v2_IN is not Null Then
		If v2_IN_issuedBy is not Null Then
			v_buffer := v_buffer || chr(9) || chr(9) || chr(9) || '<sfa_ftc:TIN issuedBy = "' || v2_IN_issuedBy || '">' || v2_IN || '</sfa_ftc:TIN>' || chr(10);
	    Else
			v_buffer := v_buffer || chr(9) || chr(9) || chr(9) || '<sfa_ftc:TIN>' || v2_IN || '</sfa_ftc:TIN>' || chr(10);
		End If;
	End If;
	...
	End If;
	If 	v2_Street is not null
		or v2_BuildingIdentifier is not null
		or v2_SuiteIdentifier is not null
		or v2_FloorIdentifier is not null
		or v2_DistrictName is not null
		or v2_POB is not null
		or v2_PostCode is not null
		or v2_City is not null
	Then
	...
	LOOP
			...
			If v3_AccountNumber is not Null Then
				v_buffer := v_buffer || chr(9) || chr(9) || chr(9) || chr(9) || '<sfa_ftc:AccountNumber';
				If v3_UndocumentedAccount = 'true' Then
					v_buffer := v_buffer || ' UndocumentedAccount = "true"';
				End If;
				If v3_ClosedAccount = 'true' Then
					v_buffer := v_buffer || ' ClosedAccount = "true"';
				End If;
				If v3_DormantAccount = 'true' Then
					v_buffer := v_buffer || ' DormantAccount = "true"';
				End If;
				v_buffer := v_buffer || '>' || v3_AccountNumber || '</sfa_ftc:AccountNumber>' || chr(10);
			End If;
			...
			If v3_IN is not Null Then
				If v3_IN_issuedBy is not Null Then
					v_buffer := v_buffer || chr(9) || chr(9) || chr(9) || chr(9) || chr(9) || chr(9) || '<sfa_ftc:TIN issuedBy = "' || v3_IN_issuedBy || '">' || v3_IN || '</sfa_ftc:TIN>' || chr(10);
				Else
					v_buffer := v_buffer || chr(9) || chr(9) || chr(9) || chr(9) || chr(9) || chr(9) || '<sfa_ftc:TIN>' || v3_IN || '</sfa_ftc:TIN>' || chr(10);
				End If;
			End If;
			...
			If v3_TypeHolder = 'Individual' Then
				...
				Else
					v_buffer := v_buffer || chr(9) || chr(9) || chr(9) || chr(9) || chr(9) || chr(9) || '<sfa_ftc:Name nameType = "' || v3_Name_nameType || '">' || chr(10);
				End If;
			...
			End If;
			If 		v3_Street is not null 
					or v3_BuildingIdentifier is not null
					or v3_SuiteIdentifier is not null
					or v3_FloorIdentifier is not null
					or v3_DistrictName is not null
					or v3_POB is not null
					or v3_PostCode is not null
					or v3_City is not null
			Then
			...
			LOOP
				...
				If v4_IN is not Null Then
					If v4_IN_issuedBy is not Null Then
						v_buffer := v_buffer || chr(9) || chr(9) || chr(9) || chr(9) || chr(9) || chr(9) || '<sfa_ftc:TIN issuedBy = "' || v4_IN_issuedBy || '">' || v4_IN || '</sfa_ftc:TIN>' || chr(10);
					Else
						v_buffer := v_buffer || chr(9) || chr(9) || chr(9) || chr(9) || chr(9) || chr(9) || '<sfa_ftc:TIN>' || v4_IN || '</sfa_ftc:TIN>' || chr(10);
					End If;
				End If;
